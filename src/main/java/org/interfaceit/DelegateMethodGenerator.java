...
public class DelegateMethodGenerator {
	...
	private String makeGenericMarker(Method method) {
		...
		Optional<String> genericPrefix = Arrays.stream(split).filter(s -> s.startsWith("<") && s.endsWith(">"))
				.findFirst();
	...
	public String generateDelegateClassCode(String targetPackageName, String targetInterfaceName,
		...
		buf.append(NEWLINE).append("/** ").append(NEWLINE).append(" * Wrapper of static elements in ").append(delegateClass.getTypeName())
		.append(NEWLINE).append(" * Generated by Interface-It: https://github.com/aro-tech/interface-it")
		.append(NEWLINE).append(" * {@link ").append(delegateClass.getTypeName()).append("}")
		.append(NEWLINE).append(" */").append(NEWLINE).append("public interface ").append(targetInterfaceName).append(" {").append(NEWLINE)
